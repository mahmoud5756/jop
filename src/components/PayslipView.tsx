...
export const PayslipView: React.FC<PayslipViewProps> = ({ employee, onBack }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/80 backdrop-blur-xs flex flex-col items-center p-2 sm:p-6 print:p-0 print:bg-white print:static print:overflow-visible">
        ...
        <div className="p-6 sm:p-8 print:p-0 min-h-[297mm] print:min-h-[297mm] print:h-[297mm] print-page flex flex-col justify-between relative">
          <div>
            ...
            <div className="flex items-start justify-between border-b-2 border-[#9E1A24] pb-3 mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-xl font-black text-[#9E1A24] tracking-tight">مفردات المرتب</h1>
                  <h2 className="text-lg font-bold text-stone-900 tracking-wider">BOB WICH</h2>
                  <p className="text-[11px] text-stone-500 font-semibold mt-0.5">قسيمة راتب شهرية</p>
                </div>
              </div>
            ...
            {/* Employee Data */}
            <div className="mb-4">
              <div className="bg-[#9E1A24] text-white px-2.5 py-0.5 text-[11px] font-bold rounded-t">بيانات الموظف</div>
            ...
            {/* Earnings Table */}
            <div className="mb-4">
              <div className="bg-emerald-700 text-white px-2.5 py-0.5 text-[11px] font-bold rounded-t">المستحقات</div>
              <table className="w-full text-xs border border-stone-300 border-t-0">
                <tbody>
                  <tr className="border-b border-stone-200">
                    <td className="p-2 font-semibold text-stone-700">الراتب الأساسي</td>
                    <td className="p-2 font-mono text-left">{fmt(num(basicSalary))} ج.م</td>
                  </tr>
                  <tr className="border-b border-stone-200 bg-stone-50">
                    <td className="p-2 font-semibold text-stone-700">البدلات</td>
                    <td className="p-2 font-mono text-left">{fmt(num(allowances))} ج.م</td>
                  </tr>
                  <tr className="border-b border-stone-200">
                    <td className="p-2 font-semibold text-stone-700">الحوافز/المكافآت</td>
                    <td className="p-2 font-mono text-left">{fmt(num(bonuses))} ج.م</td>
                  </tr>
                  <tr className="border-b border-stone-200 bg-stone-50">
                    <td className="p-2 font-semibold text-stone-700">مقابل ساعات إضافية</td>
                    <td className="p-2 font-mono text-left">{fmt(num(overtime))} ج.م</td>
                  </tr>
                  <tr className="bg-emerald-50">
                    <td className="p-2 font-black text-emerald-800">إجمالي المستحقات</td>
                    <td className="p-2 font-mono font-black text-emerald-800 text-left">{fmt(totals.gross)} ج.م</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            {/* Deductions Table */}
            <div className="mb-4">
              <div className="bg-red-700 text-white px-2.5 py-0.5 text-[11px] font-bold rounded-t">الاستقطاعات</div>
              <table className="w-full text-xs border border-stone-300 border-t-0">
                <tbody>
                  <tr className="border-b border-stone-200">
                    <td className="p-2 font-semibold text-stone-700">التأمينات الاجتماعية</td>
                    <td className="p-2 font-mono text-left">{fmt(num(socialInsurance))} ج.م</td>
                  </tr>
                  <tr className="border-b border-stone-200 bg-stone-50">
                    <td className="p-2 font-semibold text-stone-700">خصم غياب/تأخير</td>
                    <td className="p-2 font-mono text-left">{fmt(num(absenceDeduction))} ج.م</td>
                  </tr>
                  <tr className="border-b border-stone-200">
                    <td className="p-2 font-semibold text-stone-700">سلف مستحقة</td>
                    <td className="p-2 font-mono text-left">{fmt(num(advanceDeduction))} ج.م</td>
                  </tr>
                  <tr className="border-b border-stone-200 bg-stone-50">
                    <td className="p-2 font-semibold text-stone-700">جزاءات أخرى</td>
                    <td className="p-2 font-mono text-left">{fmt(num(otherPenalties))} ج.م</td>
                  </tr>
                  <tr className="bg-red-50">
                    <td className="p-2 font-black text-red-800">إجمالي الاستقطاعات</td>
                    <td className="p-2 font-mono font-black text-red-800 text-left">{fmt(totals.totalDeductions)} ج.م</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            {/* Net Pay */}
            <div className="bg-[#9E1A24] text-white rounded-xl p-4 flex items-center justify-between mb-4">
              <span className="font-black text-sm">الصافي المستحق للصرف</span>
              <span className="font-mono font-black text-xl">{fmt(totals.net)} ج.م</span>
            </div>

            <p className="text-[10px] text-stone-500 leading-relaxed">
              هذه القسيمة صادرة إلكترونيًا وتوضح تفاصيل المستحقات والاستقطاعات الخاصة بالشهر المذكور أعلاه، ولا تُعتبر إقرارًا نهائيًا إلا بعد توقيع الاستلام أدناه.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-8 mt-8 pt-6 border-t-2 border-stone-300">
            <div className="text-center space-y-6">
              <p className="font-bold text-xs text-stone-800">استلمت المبلغ المذكور أعلاه بالكامل</p>
              <div className="text-[11px] space-y-3">
                <p>الاسم: {employee.full_name || '____________________'}</p>
                <p className="pt-5 border-t border-stone-400 mt-5">التوقيع والتاريخ: __________</p>
              </div>
            </div>
            <div className="text-center space-y-6">
              <p className="font-bold text-xs text-stone-800">الموارد البشرية / الحسابات</p>
              <div className="text-[11px] space-y-3">
